...
(cl:defmethod cl:initialize-instance :after ((m <NavCmd>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'NavCmd)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name nav_command-msg:<NavCmd> is deprecated: use nav_command-msg:NavCmd instead.")))

(cl:ensure-generic-function 'start_nav-val :lambda-list '(m))
(cl:defmethod start_nav-val ((m <NavCmd>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader nav_command-msg:start_nav-val is deprecated.  Use nav_command-msg:start_nav instead.")
  (start_nav m))

(cl:ensure-generic-function 'target_x-val :lambda-list '(m))
(cl:defmethod target_x-val ((m <NavCmd>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader nav_command-msg:target_x-val is deprecated.  Use nav_command-msg:target_x instead.")
  (target_x m))

(cl:ensure-generic-function 'target_y-val :lambda-list '(m))
(cl:defmethod target_y-val ((m <NavCmd>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader nav_command-msg:target_y-val is deprecated.  Use nav_command-msg:target_y instead.")
  (target_y m))

(cl:ensure-generic-function 'target_yaw-val :lambda-list '(m))
(cl:defmethod target_yaw-val ((m <NavCmd>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader nav_command-msg:target_yaw-val is deprecated.  Use nav_command-msg:target_yaw instead.")
  (target_yaw m))

(cl:ensure-generic-function 'robot_id-val :lambda-list '(m))
(cl:defmethod robot_id-val ((m <NavCmd>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader nav_command-msg:robot_id-val is deprecated.  Use nav_command-msg:robot_id instead.")
  (robot_id m))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<NavCmd>)))
  ...
  (cl:format cl:nil "# 导航指令消息~%bool start_nav       # 是否启动导航（true=启动，false=停止）~%float64 target_x     # 目标点 x 坐标（map 坐标系）~%float64 target_y     # 目标点 y 坐标（map 坐标系）~%float64 target_yaw   # 目标点偏航角（弧度）~%string robot_id      # 目标机器人 ID（区分发给 A/B）~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'NavCmd)))
  "Returns full string definition for message of type 'NavCmd"
  (cl:format cl:nil "# 导航指令消息~%bool start_nav       # 是否启动导航（true=启动，false=停止）~%float64 target_x     # 目标点 x 坐标（map 坐标系）~%float64 target_y     # 目标点 y 坐标（map 坐标系）~%float64 target_yaw   # 目标点偏航角（弧度）~%string robot_id      # 目标机器人 ID（区分发给 A/B）~%~%~%"))
