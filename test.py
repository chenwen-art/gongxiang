...
class CompetitionMission:
    ...
    def wait_for_start(self):
        ...
        rospy.loginfo("正在发送初始位姿，消除旋转误差...")
        self.set_pose_pub.publish(initial_pose)
        rospy.sleep(rospy.Duration(3000)) # 等待 AMCL 接收并处理初始位姿
        # --- 新增代码结束 ---
    ...
    def run(self):
        ...
        self.wait_for_start()
        self.set_initial_pose(x=0.0, y=0.0, yaw=0.0)  # 替换为你的实际起点
        rospy.sleep(10)  # 给 AMCL 时间收敛
...
def set_initial_pose(self, x, y, yaw_deg):
    ...
    q = transformations.quaternion_from_euler(0, 0, yaw_deg * pi / 180.0)
    msg.pose.pose.orientation.x = q[0]
    msg.pose.pose.orientation.y = q[1]
    msg.pose.pose.orientation.z = q[2]
    msg.pose.pose.orientation.w = q[3]
    # 可选：设置协方差（更精确）
    msg.pose.covariance[6*0+0] = 0.05 * 0.05  # x
    msg.pose.covariance[6*1+1] = 0.05 * 0.05  # y
    msg.pose.covariance[6*5+5] = (5.0 * pi / 180.0) ** 2  # yaw
    self.set_pose_pub.publish(msg)
    rospy.loginfo("已发布初始位姿: (%.2f, %.2f, %.1f°)", x, y, yaw_deg)
